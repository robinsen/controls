...
bool CClientSocket::start()
{
    ...
    m_thread.reset(new std::thread(std::bind(&CClientSocket::threadCallBack, nullptr, this)));
    return true;
}
...
bool CClientSocket::connectToServer(const char *ip, unsigned short port , int sec)
{
    ...
    struct sockaddr_in saddr;   //设置连接对象的结构体
            ...
            for(;*pptr != NULL && !m_bStop;pptr++)
            {
                ...
                do
                {
                    ...
                    if(m_bConnect)
                    {
                        ...
                        #else
                        int datalen = read(m_socket, dataBuf, 1024);
                        #endif
                            // 
                            if(datalen <= 0)
                            {
...
#else
    int ret = write(m_socket,pdata,len);
#endif
    return (ret > 0);
 }
...
void CClientSocket::threadCallBack(void* pData, CClientSocket* pThis)
{
    if (pThis)
        pThis->threadRun();
}